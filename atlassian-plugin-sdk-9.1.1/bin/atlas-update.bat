...
:showhelp
echo.
echo Usage: atlas-update [options]
echo.
...
if /I "%ARG%"=="--update-version" (
        set MVN_PARAMS=%MVN_PARAMS% -Dupdate.version=%ARGNEXT%
        set /a ARGI = !ARGI! + 1
        goto loopstart
)
if /I "%ARG%"=="-v" (
...
if /I "%ARG%"=="--sdk-archive-path" (
        set MVN_PARAMS=%MVN_PARAMS% -Dsdk.archive.path=%ARGNEXT%
        set /a ARGI = !ARGI! + 1
        goto loopstart
)
if /I "%ARG%"=="-s" (
...
if /I "%ARG%"=="--force-update-check" (
        set MVN_PARAMS=%MVN_PARAMS% -Dforce.update.check=%ARGNEXT%
        set /a ARGI = !ARGI! + 1
        goto loopstart
)
if /I "%ARG%"=="-f" (
...
if "%AMPS_PLUGIN_VERSION_STRIPPED%" LSS "8" (
    set MVN_PLUGIN=maven-amps-plugin
)
if "%AMPS_PLUGIN_VERSION_STRIPPED%" GEQ "8" (
    set MVN_PLUGIN=amps-maven-plugin
)

set MVN_COMMAND=com.atlassian.maven.plugins:%MVN_PLUGIN%:%AMPS_PLUGIN_VERSION_STRIPPED%:update
