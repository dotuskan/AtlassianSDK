...
:showhelp
echo.
echo Usage: atlas-remote-test [options]
echo.
...
if /I "%ARG%"=="--pdkUsername" (
        set MVN_PARAMS=%MVN_PARAMS% -Dpdk.username=%ARGNEXT%
        set /a ARGI = !ARGI! + 1
        goto loopstart
)

if /I "%ARG%"=="--pdkPassword" (
        set MVN_PARAMS=%MVN_PARAMS% -Dpdk.password=%ARGNEXT%
        set /a ARGI = !ARGI! + 1
        goto loopstart
)
...
set MVN_COMMAND=com.atlassian.maven.plugins:%MVN_PLUGIN%:%AMPS_PLUGIN_VERSION_STRIPPED%:remote-test
