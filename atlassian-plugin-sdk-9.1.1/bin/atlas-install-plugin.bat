...
if /I "%ARG%"=="--username" (
        set MVN_PARAMS=%MVN_PARAMS% -Dusername=%ARGNEXT%
        set /a ARGI = !ARGI! + 1
        goto loopstart
)

if /I "%ARG%"=="--password" (
        set MVN_PARAMS=%MVN_PARAMS% -Dpassword=%ARGNEXT%
        set /a ARGI = !ARGI! + 1
        goto loopstart
)

if /I "%ARG%"=="--plugin-key" (
        set MVN_PARAMS=%MVN_PARAMS% -Datlassian.plugin.key=%ARGNEXT%
        set /a ARGI = !ARGI! + 1
        goto loopstart
)
...
set MVN_COMMAND=com.atlassian.maven.plugins:%MVN_PLUGIN%:%AMPS_PLUGIN_VERSION_STRIPPED%:install
